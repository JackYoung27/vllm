...
class ServingScores(OpenAIServing):
    ...
    async def _late_interaction_score(
        ...
        Queries and documents are encoded independently. Query embeddings are
        cached on the worker, and document requests return scalar MaxSim scores
        directly to avoid transferring token embeddings back to the server.
        """
        model_config = self.model_config
        tokenizer = self.renderer.get_tokenizer()
        tokenization_kwargs = request.build_tok_params(model_config).get_encode_kwargs()
